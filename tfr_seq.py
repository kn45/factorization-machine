...
def write_tfrecord2():
    ...
    def make_example(sequence, labels):
        feature = {'label':  _int64_feature(labels),
                   'indices':  _int64_feature(sequence),
                   'length':   _int64_feature([len(sequence)])}

        # The object we return
        ex = tf.train.SequenceExample(feature_lists={'feature_list': feature})
        return ex
    ...
    for ln in reader:
        flds = ln.rstrip('\n').split('\t')
        sequence = list(map(int, flds[1:]))
        label_sequence = list(map(int, flds[0:1]))
